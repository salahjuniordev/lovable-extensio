// ============================================
// Lovable Pro – Business Logic (content)
  ...
  return new Promise(function(resolve) {
    chrome.storage.local.get(["ql_license_valid", "ql_session_id", "ql_user_name", "ql_license_key", "lovable_user_name"], function(res) {
      if (res.ql_license_valid && res.ql_session_id) {
        ...
        qlUserName = normalizeLicenseUserName(res.lovable_user_name || res.ql_user_name);
...
let qlUserName = null;
let qlCustomName = null;
let qlProjectName = null;
...
function _buildFloatingUI(){
  ...
  chrome.storage.local.get(["ql_license_valid","ql_license_key","ql_minimized","ql_height","ql_dark_mode","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_session_id","ql_custom_name","lovable_project_name"], async (res) => {
    qlCustomName = res.ql_custom_name || null;
    qlProjectName = res.lovable_project_name || null;
...
function showMainUI(box){
  const greeting = qlCustomName || normalizeLicenseUserName(qlUserName);
...
function setupWelcomeNote() {
  ...
  var NAME_KEY = "ql_custom_name";
  chrome.storage.local.get([ONBOARD_KEY, NAME_KEY], function(res) {
    if (res[ONBOARD_KEY]) return;
    if (document.getElementById("ql-welcome-overlay")) return;
    const box = document.getElementById("ql-floating");
    if (!box) return;
    const div = document.createElement("div");
    div.innerHTML = templateWelcomeNote();
    box.appendChild(div.firstElementChild);
    const overlay = document.getElementById("ql-welcome-overlay");
    const nameInput = document.getElementById("ql-welcome-name");

    const finish = function() {
      var name = nameInput ? nameInput.value.trim() : "";
      var patch = {};
      patch[ONBOARD_KEY] = true;
      if (name) { patch[NAME_KEY] = name; qlCustomName = name; }
      chrome.storage.local.set(patch);
      var nameEl = document.querySelector(".ql-profile-name");
      if (nameEl && name) nameEl.textContent = name;
      overlay.remove();
    };

    document.getElementById("ql-welcome-start").onclick = finish;
    document.getElementById("ql-welcome-close").onclick = finish;
    if (nameInput) {
      nameInput.addEventListener("keydown", function(e) { if (e.key === "Enter") finish(); });
...
function updateProjectSection() {
  chrome.storage.local.get(["lovable_project_name", "lovable_projectId"], function(res) {
    var bar = document.getElementById("ql-project-bar");
    var nameEl = document.getElementById("ql-project-name");
    if (!bar || !nameEl) return;
    var name = res.lovable_project_name || null;
    var id = res.lovable_projectId || null;
    if (name) {
      nameEl.textContent = name;
      bar.style.display = "flex";
    } else if (id) {
      nameEl.textContent = id.substring(0, 8) + "…";
      bar.style.display = "flex";
    } else {
...
function setupWatermarkButton(){
  ...
  btn.addEventListener("click", async function(){
    ...
    btn.disabled = true;
    btn.innerHTML = SVG_ICONS.clock + " Sending...";
...
function showPublishedUrlModal(url){
  ...
  overlay.innerHTML =
    '<div class="pk-publish-modal pk-publish-modal-wide">' +
      '<div class="pk-publish-emoji" style="color:#667eea"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg></div>' +
      ...
      '<div class="pk-publish-actions">' +
        '<button id="ql-publish-copy" class="pk-publish-copy">' + SVG_ICONS.clipboard + ' Copy</button>' +
        '<button id="ql-publish-open" class="pk-publish-open">' + SVG_ICONS.link + ' Open</button>' +
      '</div>' +
  ...
  document.getElementById("ql-publish-copy").addEventListener("click", function(){
    ...
    this.innerHTML = SVG_ICONS.checkCircle + " Copied!";
  });
...
function setupPublishProject(){
  ...
  btn.addEventListener("click", async function(){
    var log = document.getElementById("ql-log");
    btn.disabled = true;
    btn.innerHTML = SVG_ICONS.clock + " Publishing...";
    ...
    if(!projectId || !token){
      ...
      btn.disabled = false;
      btn.innerHTML = SVG_ICONS.globe + " Publish Project";
      return;
    }
...
function setupEnableCloud(){
  ...
  btn.addEventListener("click", async function(){
    var log = document.getElementById("ql-log");
    btn.disabled = true;
    btn.innerHTML = SVG_ICONS.clock + " Activating Cloud...";
    ...
    if(!projectId || !token){
      if(log){ log.className = "ql-log-error"; log.innerText = "Project not synced."; }
      btn.disabled = false;
      btn.innerHTML = SVG_ICONS.cloud + " Enable Lovable Cloud";
      return;
    }
...
function updateTrialCountdown(){
  ...
  if(!qlExpiresAt) {
    if (qlValidityMinutes) {
      ...
      el.innerHTML = '<div class="ql-countdown-row"><span class="ql-countdown-icon">' + SVG_ICONS.clock + '</span><span class="ql-countdown-label">Trial ready:</span><span class="ql-countdown-time">' + qlValidityMinutes + ' min after activation</span></div>';
    } else {
...
function showModoPlanoAlert(){
  ...
  overlay.innerHTML = '<div class="ql-modo-plano-modal">' +
    '<div class="ql-modo-plano-icon">' + SVG_ICONS.alertTriangle + '</div>' +
...
function setupShield(){
  ...
  btn.addEventListener("click", () => {
    ...
    if(qlShieldActive) {
      ...
      if(label) label.textContent = "Disable Shield";
      injectShieldOverlay();
      showCustomAlert("Shield Enabled", "The Lovable input is locked. Use the extension to send prompts.");
    } else {
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text">' + SVG_ICONS.shield + ' Protected by Lovable Pro</span>' +
...
async function showPaymentUI(box, preselectedPkg){
  ...
  document.querySelectorAll(".ql-brl-buy").forEach(function(btn){
    btn.addEventListener("click", function(){
      ...
      if(!plan) return;
      var msg = "Hello! I am interested in the *" + plan.name + "* plan for Lovable Pro (R$ " + plan.price + " - " + plan.period + ").\n\nPlease provide more information.";
      var url = DISCORD_URL;
  ...
  try {
    ...
    if(!packages || !Array.isArray(packages) || packages.length === 0){
      list.innerHTML = '<div class="ql-pay-loading">' + SVG_ICONS.clock + ' Loading plans...</div>';
      return;
    }
    ...
    const list = document.getElementById("ql-packages-list");
    if(list) list.innerHTML = '<div class="ql-pay-loading">' + SVG_ICONS.clock + ' Loading plans...</div>';
  }
}
...
function showCheckoutScreen(box, pkg){
  ...
  if(confirmBtn){
    confirmBtn.addEventListener("click", async () => {
      ...
      if(!phoneInput){
        if(log){ log.className = "ql-pay-log ql-pay-info"; log.textContent = "Please contact support to complete your purchase."; }
        return;
      }
      ...
      confirmBtn.innerHTML = SVG_ICONS.clock + " Processing...";
      ...
      try {
        ...
        if(result && result.status === "sucesso"){
            ...
            if(copyBtn){
              copyBtn.addEventListener("click", () => {
                navigator.clipboard.writeText(result.license_key).then(() => {
                  copyBtn.innerHTML = SVG_ICONS.checkCircle + " Copied!";
                  setTimeout(() => { copyBtn.innerHTML = SVG_ICONS.clipboard + " Copy Key"; }, 2000);
                }).catch(() => {
          ...
          const errMsg = (result && result.error) ? result.error : "Activation failed. Please try again.";
          if(log){ log.className = "ql-pay-log ql-pay-error"; log.textContent = errMsg; }
          confirmBtn.disabled = false;
          confirmBtn.innerHTML = SVG_ICONS.key + " Activate Now";
        }
      } catch(err) {
  ...
  chrome.runtime.sendMessage({
    ...
    chrome.storage.local.get(["lovable_projectId","lovable_token"], (res)=>{
      ...
      if(res.lovable_projectId && token && isTokenFresh(token)){
        ...
        status.innerHTML = '<span class="ql-sync-text">' + SVG_ICONS.checkCircle + ' Synced! Project: ' + pid + '...</span>';
      } else if (res.lovable_projectId && token) {
        status.className = "ql-sync-status ql-sync-waiting";
        status.innerHTML = '<span class="ql-sync-text">' + SVG_ICONS.alertTriangle + ' Log in on lovable.dev</span>';
      } else {
        status.className = "ql-sync-status ql-sync-waiting";
        status.innerHTML = '<span class="ql-sync-text">' + SVG_ICONS.clock + ' Waiting for sync...</span>';
      }
    });
  });
}
...
function renderHistoryView() {
  ...
  if(!qlChatHistory.length) {
    container.innerHTML = '<div class="ql-chat-empty"><div style="margin-bottom:8px">' + SVG_ICONS.msgSquare + '</div><div style="font-size:13px;font-weight:600;color:var(--ql-text-primary,#f4f4f5)">No messages</div><div style="font-size:11px;color:var(--ql-text-muted,#71717a);margin-top:4px">Your sent prompts will appear here.</div></div>';
    return;
  }
  ...
  for(let i = 0; i < qlChatHistory.length; i++) {
    ...
    const statusText = m.status === 'error' ? SVG_ICONS.xMark + ' Error' : SVG_ICONS.check + ' Sent';
  ...
  html += '</div>';
  html += '<div class="ql-chat-actions"><span class="ql-chat-count">' + qlChatHistory.length + ' message' + (qlChatHistory.length === 1 ? '' : 's') + '</span><button class="ql-chat-clear" id="ql-chat-clear">' + SVG_ICONS.trash + ' Clear</button></div>';
...
function renderAttachPreview() {
  ...
  container.innerHTML = qlAttachedFiles.map((f, i) => {
    ...
    return '<div class="ql-attach-item' + uploadingClass + '" data-idx="' + i + '">' +
      ...
      '<button class="ql-attach-remove" data-idx="' + i + '">' + SVG_ICONS.xMark + '</button>' +
    '</div>';
  }).join('');
...
function setupClipboardPaste() {
  ...
  function showDragOverlay() {
    ...
    dragOverlay.innerHTML = '<div class="ql-drag-overlay-inner">' + SVG_ICONS.folder + ' Drop files here</div>';
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = SVG_ICONS.zap + " <span>Lovable Pro</span>";
  ...
  if (!document.getElementById("ql-native-return-btn")) {
    ...
    returnBtn.innerHTML = SVG_ICONS.arrowLeft + " Return to Extension";
...
window.addEventListener("message", (event)=>{
  ...
  if(event.data.token && typeof event.data.token === "string"){
    ...
    try {
      var _p = JSON.parse(atob(updates.lovable_token.split('.')[1].replace(/-/g,'+').replace(/_/g,'/')));
      var _n = (_p.user_metadata && (_p.user_metadata.full_name || _p.user_metadata.name)) || (_p.email && _p.email.split('@')[0]) || null;
      if (_n) { updates.lovable_user_name = _n; qlUserName = normalizeLicenseUserName(_n); var _el = document.querySelector('.ql-profile-name'); if(_el) _el.textContent = qlUserName; }
    } catch(_e) {}
  }
  if(event.data.projectId && typeof event.data.projectId === "string"){
    ...
    try {
      ...
      var projName = rawTitle
        .replace(/\s*[|·–—]\s*lovable\s*/gi, "")
        .replace(/\s*lovable\s*[|·–—]\s*/gi, "")
        .replace(/^lovable\s*[-–]\s*/i, "")
        .replace(/\s*[-–]\s*lovable$/i, "")
        .trim();
      if (projName && projName.toLowerCase() !== "lovable") {
        updates.lovable_project_name = projName;
        qlProjectName = projName;
      }
    } catch(_e) {}
  }
