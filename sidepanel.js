// ============================================================
// Lovable Pro - Side Panel Logic (Business Logic Only)
...
(function(){
  ...
  function ensureInternalSessionLocal() {
    ...
    return new Promise(function(resolve) {
      chrome.storage.local.get(["ql_license_valid", "ql_session_id", "ql_user_name", "ql_license_key", "lovable_user_name"], function(res) {
        if (res.ql_license_valid && res.ql_session_id) {
          sessionId = res.ql_session_id;
          userName = normalizeLicenseUserName(res.lovable_user_name || res.ql_user_name);
  ...
  let sessionId = null, userName = null, expiresAt = null, licenseStatus = null, validityMinutes = null, spActivatedAt = null, heartbeatInterval = null, deviceId = null, isResellerUser = false, spCustomName = null, spProjectName = null;
  ...
  // --- Main UI ---
  function showMainUI() {
    const greeting = spEscapeHtml(spCustomName || normalizeLicenseUserName(userName));
    const statusBadge = spTemplateStatusBadge(licenseStatus);
    const body = document.getElementById('sp-body');
    loadChatHistory(function() {
      var projectBarHtml = spProjectName
        ? '<div class="sp-project-bar" id="sp-project-bar"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg><span class="sp-project-name" id="sp-project-name">' + spEscapeHtml(spProjectName) + '</span></div>'
        : '<div class="sp-project-bar" id="sp-project-bar" style="display:none"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg><span class="sp-project-name" id="sp-project-name"></span></div>';
      body.innerHTML = '<div id="sp-update-banner" style="display:none"></div>' +
        '<div class="sp-agent-bar">' +
          '<div class="sp-agent-left"><div class="sp-agent-dot"></div><span class="sp-profile-name" id="sp-name">' + greeting + '</span></div>' +
          statusBadge +
        '</div>' +
        projectBarHtml +
        '<div class="sp-sync-status" id="sp-sync">' + SP_SVG.clock + ' Waiting for sync...</div>' +
      ...
      chrome.storage.onChanged.addListener((ch) => {
        ...
        if(ch.lovable_user_name && ch.lovable_user_name.newValue) {
          userName = normalizeLicenseUserName(ch.lovable_user_name.newValue);
          const el = document.getElementById('sp-name'); if(el) el.textContent = userName;
        }
      });
    ...
    container.innerHTML =
      '<div class="sp-composer">' +
        ...
        '<div class="sp-composer-bar">' +
          '<div class="sp-composer-left"><label class="sp-toggle"><input type="checkbox" id="sp-modo-plano"><span class="sp-toggle-slider"></span></label><span class="sp-toggle-label">Plan</span></div>' +
          '<div class="sp-composer-sep"></div>' +
      ...
      '<button type="button" class="sp-advanced-toggle" id="sp-advanced-toggle" aria-expanded="false" aria-controls="sp-advanced-panel">' +
        '<span class="sp-advanced-toggle-label">' + SP_SVG.wrench + ' Advanced Options</span>' +
  ...
  function renderSyncFromStorage(r) {
    ...
    if(r.lovable_projectId && token && isTokenFresh(token)) {
      ...
      el.innerHTML = SP_SVG.checkCircle + ' Synced! Project: ' + r.lovable_projectId.substring(0,6) + '...';
    } else if (r.lovable_projectId && token) {
      el.className = 'sp-sync-status sp-sync-waiting';
      el.innerHTML = SP_SVG.alertTriangle + ' Log in on lovable.dev and open your project';
    } else {
      el.className = 'sp-sync-status sp-sync-waiting';
      el.innerHTML = SP_SVG.clock + ' Open lovable.dev on your project tab';
    }
  }
  ...
  function updateCountdown() {
    ...
    if(!expiresAt) {
      if (validityMinutes && el) {
        ...
        el.innerHTML = '<span style="color:var(--ql-text-muted);font-size:12px">' + SP_SVG.clock + ' Trial: ' + validityMinutes + ' min after activation</span>';
      } else if (el) {
  ...
  function showModoPlanoAlert() {
    ...
    overlay.innerHTML = '<div class="sp-modal">' +
      '<div class="sp-modal-icon">' + SP_SVG.alertTriangle + '</div>' +
  ...
  function showSpPublishedUrlModal(url){
    ...
    overlay.innerHTML =
      '<div class="pk-publish-modal">' +
        '<div class="pk-publish-emoji" style="color:#667eea"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg></div>' +
        ...
        '<div class="pk-publish-actions">' +
          '<button id="sp-publish-copy" class="pk-publish-copy">' + SP_SVG.clipboard + ' Copy</button>' +
          '<button id="sp-publish-open" class="pk-publish-open">' + SP_SVG.link + ' Open</button>' +
        '</div>' +
    ...
    document.getElementById("sp-publish-copy").addEventListener("click", function(){
      ...
      this.innerHTML = SP_SVG.checkCircle + " Copied!";
    });
  ...
  function setupSpPublishProject(){
    ...
    btn.addEventListener("click", async function(){
      ...
      btn.disabled = true;
      btn.innerHTML = SP_SVG.clock + " Publishing...";
      ...
      try {
        ...
        btn.disabled = false;
        btn.innerHTML = SP_SVG.globe + " Publish Project";
      }
    });
  }
  ...
  function setupSpEnableCloud(){
    ...
    btn.addEventListener("click", async function(){
      var log = document.getElementById("sp-log");
      btn.disabled = true;
      btn.innerHTML = SP_SVG.clock + " Activating Cloud...";
      ...
      try {
        ...
        btn.disabled = false;
        btn.innerHTML = SP_SVG.cloud + " Enable Lovable Cloud";
      }
    });
  }
  ...
  function setupSpWatermarkButton(){
    ...
    btn.addEventListener("click", async function(){
      var log = document.getElementById("sp-log");
      btn.disabled = true;
      btn.innerHTML = SP_SVG.clock + " Sending...";
  ...
  async function handleSend() {
    ...
    btn.disabled = true; btn.innerHTML = SP_SVG.clock;
  ...
  function setupSpClipboardPaste() {
    ...
    function showDragOverlay() {
      ...
      dragOverlay.innerHTML = '<div class="sp-drag-overlay-inner">' + SP_SVG.folder + ' Drop files here</div>';
  ...
  function setupSpDownloadProject() {
    ...
    btn.addEventListener('click', async function() {
      ...
      btn.disabled = true;
      btn.innerHTML = SP_SVG.refresh + ' Preparing...';
      ...
      try {
        ...
        btn.innerHTML = SP_SVG.download + ' Downloading...';
        ...
        btn.innerHTML = SP_SVG.pkg + ' Packaging...';
        ...
        if (statusEl) statusEl.textContent = 'Compressing ' + addedFiles + ' files...';
        ...
        btn.innerHTML = SP_SVG.checkCircle + ' Download Complete!';
        setTimeout(function() {
          btn.innerHTML = SP_SVG.download + ' Download All Files';
          btn.disabled = false;
        ...
        btn.innerHTML = SP_SVG.xCircle + ' Failed';
        setTimeout(function() {
          btn.innerHTML = SP_SVG.download + ' Download All Files';
          btn.disabled = false;
        }, 3000);
  ...
  // --- Welcome Note ---
  var WELCOME_KEY = "ql_welcome_seen";

  function setupWelcomeNote() {
    chrome.storage.local.get([WELCOME_KEY], function(res) {
      if (res[WELCOME_KEY]) return;
      if (document.getElementById("sp-welcome-overlay")) return;

      var overlay = document.createElement("div");
      overlay.id = "sp-welcome-overlay";
      overlay.className = "sp-modal-overlay";
      overlay.innerHTML =
        '<div class="sp-modal" style="text-align:center;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);border:1px solid rgba(255,255,255,0.08)">' +
          '<div style="margin-bottom:14px;color:#667eea"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/><path d="M5 3v4"/><path d="M19 17v4"/><path d="M3 5h4"/><path d="M17 19h4"/></svg></div>' +
          '<div class="sp-modal-title" style="font-size:16px;color:#fff;font-weight:700">Welcome to Lovable Pro</div>' +
          '<div style="font-size:11px;color:rgba(255,255,255,0.38);margin:3px 0 14px;letter-spacing:0.6px;text-transform:uppercase">by Shoaib Ahmed</div>' +
          '<div class="sp-modal-body" style="font-size:12px;color:rgba(255,255,255,0.7);margin-bottom:20px;line-height:1.65">' +
            "You're all set! Send prompts, attach files, and optimize your Lovable projects — right from this panel." +
          '</div>' +
          '<button id="sp-welcome-start" style="display:inline-block;background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:11px 28px;border-radius:25px;border:none;font-weight:700;font-size:13px;cursor:pointer;box-shadow:0 4px 14px rgba(102,126,234,0.4)">Get Started</button>' +
          '<div style="margin-top:12px">' +
            '<button id="sp-welcome-later" style="background:none;border:none;color:rgba(255,255,255,0.35);text-decoration:underline;cursor:pointer;font-size:12px;font-family:inherit">Close</button>' +
          '</div>' +
        '</div>';
      ...
      var dismiss = function() {
        var patch = {};
        patch[WELCOME_KEY] = true;
        chrome.storage.local.set(patch);
        overlay.remove();
      };

      document.getElementById("sp-welcome-start").onclick = dismiss;
      document.getElementById("sp-welcome-later").onclick = dismiss;
  ...
  (async function init() {
    ...
    chrome.storage.local.get(["ql_license_valid","ql_license_key","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_session_id","ql_custom_name","lovable_project_name"], async (res) => {
      if (INTERNAL_LICENSE_MODE || res.ql_license_valid) {
        ...
        spCustomName = res.ql_custom_name || null;
        spProjectName = res.lovable_project_name || null;
        syncCreditBypassOnLovableTabs(true);
        showMainUI();
      } else {
        showLicenseGate();
      ...
      // Show welcome note on first open
      if (typeof setupWelcomeNote === "function") {
        setupWelcomeNote();
      }
    });
  })();
  ...
  function setupSpShield() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (spShieldActive) {
        ...
        if (label) label.textContent = 'Disable Shield';
        applySpShieldOnTab(true);
        showAlert('Shield Enabled', 'The Lovable input is locked.');
      } else {
